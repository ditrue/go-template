...
// @BasePath                    /
func main() {
	// 解析通用配置参数（如 -c 已在 core.Viper 内部解析）
	var runCmd string
	flag.StringVar(&runCmd, "cmd", "", "run command mode, e.g. importExcel")
	// 注意：core.Viper 会调用 flag.Parse，这里避免重复调用导致冲突，因此不再调用 flag.Parse
